...
import com.furkqn.music.utils.PlaylistExporter
import com.furkqn.music.utils.getExportFileUri
import com.furkqn.music.utils.saveToPublicDocuments
import kotlinx.coroutines.launch

@Composable
fun eatThisPlaylistMenuItem(onClick: () -> Unit): Material3MenuItemData =
    Material3MenuItemData(
        title = { Text(stringResource(R.string.eat_this_playlist)) },
        description = { Text(stringResource(R.string.eat_this_playlist_desc)) },
        icon = {
            Icon(
...
fun LocalPlaylistMenu(
    ...
    val menuItems =
        buildList {
            ...
            if (!isGuest && songs.size >= 2) {
                add(
                    Material3MenuItemData(
                        title = { Text(stringResource(R.string.playlist_smooth_mix)) },
                        description = { Text(stringResource(R.string.playlist_smooth_mix_desc)) },
                        icon = {
                            Icon(
            ...
            add(
                Material3MenuItemData(
                    ...
                    onClick = {
                        val shareText =
                            if (isYouTubePlaylist) {
                                com.furkqn.music.utils.PlaylistShareUtils.youTubePlaylistUrl(
                                    playlist.playlist.browseId!!,
                                )
                            } else {
                                buildString {
                                    append(
                                        com.furkqn.music.utils.PlaylistShareUtils.localPlaylistDeepLink(
                                            playlist.id,
                                        ),
                                    )
                                    if (songs.isNotEmpty()) {
    ...
    if (showExportDialog) {
        ...
        val playlistSongs =
            songs.map { song ->
                PlaylistSong(
                    map =
                        com.furkqn.music.db.entities.PlaylistSongMap(
                            songId = song.id,
