...
import com.furkqn.music.constants.AlbumSortType
import com.furkqn.music.constants.ArtistSongSortType
import com.furkqn.music.constants.ArtistSortType
...
import com.furkqn.music.db.entities.AlbumArtistMap
import com.furkqn.music.db.entities.AlbumEntity
import com.furkqn.music.db.entities.AlbumWithSongs
...
import com.furkqn.music.db.entities.EventWithSong
import com.furkqn.music.db.entities.FormatEntity
import com.furkqn.music.db.entities.LyricsEntity
import com.furkqn.music.db.entities.PlayCountEntity
...
import com.furkqn.music.db.entities.PodcastEntity
import com.furkqn.music.db.entities.RecognitionHistory
import com.furkqn.music.db.entities.RelatedSongMap
import com.furkqn.music.db.entities.SearchHistory
import com.furkqn.music.db.entities.SetVideoIdEntity
import com.furkqn.music.db.entities.Song
import com.furkqn.music.db.entities.SongAlbumMap
import com.furkqn.music.db.entities.SongArtistMap
import com.furkqn.music.db.entities.SongEntity
import com.furkqn.music.db.entities.SongWithStats
import com.furkqn.music.extensions.reversed
import com.furkqn.music.extensions.toSQLiteQuery
...
interface DatabaseDao {
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            "(SELECT s.thumbnailUrl FROM playlist_song_map psm INNER JOIN song s ON psm.songId = s.id " +
            "WHERE psm.playlistId = playlist.id ORDER BY psm.position ASC LIMIT 1) AS firstSongThumbnail, " +
            "(SELECT GROUP_CONCAT(thumbnailUrl, char(31)) FROM (" +
            "SELECT s.thumbnailUrl AS thumbnailUrl FROM playlist_song_map psm " +
            "INNER JOIN song s ON psm.songId = s.id " +
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE bookmarkedAt IS NOT NULL ORDER BY rowId",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE bookmarkedAt IS NOT NULL ORDER BY lastUpdateTime",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE bookmarkedAt IS NOT NULL ORDER BY name",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE isEditable AND bookmarkedAt IS NOT NULL ORDER BY name",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE bookmarkedAt IS NOT NULL ORDER BY songCount",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE id = :playlistId",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE isEditable AND bookmarkedAt IS NOT NULL ORDER BY rowId",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE browseId = :browseId",
    )
    ...
    @Query(
        "SELECT *, (SELECT COUNT(*) FROM playlist_song_map WHERE playlistId = playlist.id) AS songCount, " +
            ...
            "WHERE psm.playlistId = playlist.id AND s.thumbnailUrl IS NOT NULL AND s.thumbnailUrl != '' " +
            "ORDER BY psm.position ASC LIMIT 4)) AS previewThumbnailUrls " +
            "FROM playlist WHERE name LIKE '%' || :query || '%' LIMIT :previewSize",
    )
    fun searchPlaylists(
        query: String,
        previewSize: Int = Int.MAX_VALUE,
